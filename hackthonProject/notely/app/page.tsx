...
export default function HomePage() {
  ...
  const handleUpdateNote = (id: number, data: UpdateNoteData) => {
    setNotes((prev) =>
      prev.map((note) =>
        note.id === id
          ? {
  ...
  const handleCreateSubject = (data: { name: string; color: string }) => {
  ...
  const handleUpdateSubject = (id: number, data: { name?: string; color?: string }) => {
  ...
  const handleCreateSemester = (data: { name: string; start_date: string; end_date: string; is_active: boolean }) => {
              ...
              <div className="text-center space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
                  <div className="text-center p-6 rounded-lg bg-card/50 backdrop-blur-sm border">
                    ...
                    <h3 className="text-xl font-semibold mb-2">{notes.length} Notes</h3>
                    <p className="text-muted-foreground">Total notes created</p>
                  </div>
                  <div className="text-center p-6 rounded-lg bg-card/50 backdrop-blur-sm border">
                    <BookOpen className="w-12 h-12 text-secondary mx-auto mb-4" />
                    <h3 className="text-xl font-semibold mb-2">{subjects.length} Subjects</h3>
                    <p className="text-muted-foreground">Organized by subject</p>
                  </div>
                  <div className="text-center p-6 rounded-lg bg-card/50 backdrop-blur-sm border">
                    <Calendar className="w-12 h-12 text-accent mx-auto mb-4" />
                    <h3 className="text-xl font-semibold mb-2">{semesters.length} Semesters</h3>
